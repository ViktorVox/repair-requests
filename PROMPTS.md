...
Отлично, интерфейс выглядит супер! Но у нас есть небольшая проблема: стандартные компоненты Laravel Breeze остались на английском языке (страница логина, верхняя панель навигации в дашборде и выпадающее меню профиля). Давай это исправим, чтобы проект выглядел цельным.

**ЗАДАЧА НА ШАГ 3.5 (Локализация Breeze):**
Переведи тексты в стандартных Vue-компонентах Breeze на русский язык.

**ЧТО НУЖНО СДЕЛАТЬ (Выдай полные коды этих файлов с переводом):**

1. `resources/js/Pages/Auth/Login.vue`:
   - Переведи лейблы, плейсхолдеры и кнопки (Email -> Email, Password -> Пароль, Remember me -> Запомнить меня, Forgot your password? -> Забыли пароль?, Log in -> Войти).

2. `resources/js/Layouts/AuthenticatedLayout.vue`:
   - Переведи ссылки в навигации (Dashboard -> Панель управления).
   - Переведи выпадающее меню профиля (Profile -> Профиль, Log Out -> Выйти).

**ПРАВИЛА ОСТАЮТСЯ:**
Формат: [План] -> [Код файлов] -> [Git коммит] (feat: русификация базовых компонентов Breeze).



[ 2:51 - 01.03.2026 ]

Интерфейс теперь полностью на русском, выглядит отлично! Переходим к финалу.

**ЗАДАЧА НА ШАГ 4 (Финальный: Документация и Тест гонки):**
По условиям ТЗ нам нужно подготовить правильные файлы документации и скрипт для тестирования Race Condition (гонки).

**ПРАВИЛА ОСТАЮТСЯ:**
Формат: [План] -> [Код файлов] -> [Команды терминала] -> [Git коммит].

**ЧТО НУЖНО СДЕЛАТЬ:**

1. Напиши bash-скрипт `race_test.sh` (в корне проекта). 
   - Скрипт должен делать два одновременных POST запроса через `curl` к нашему эндпоинту `/requests/1/take` (для заявки ID 1, предполагаем, что она в статусе 'new').
   - Чтобы обойти CSRF и Auth при тестировании через curl, напиши в скрипте 2 команды `curl`, которые делают логин (`POST /login` с email `master1@test.com` и `master2@test.com`, получая сессионные куки в файлы `cookie1.txt` и `cookie2.txt`), а затем одновременно (`&`) бьют в эндпоинт `/requests/1/take` с этими куками.
   - Выведи HTTP статусы ответов (один должен быть успешным редиректом, второй - ошибкой или редиректом с ошибкой conflict).

2. Напиши файл `README.md` (на русском). Опиши:
   - Краткое описание проекта.
   - Как развернуть проект (через Laravel Sail: `sail up`, `sail artisan migrate --seed`, `sail npm install`, `sail npm run build`).
   - Тестовых пользователей (Диспетчер: dispatcher@test.com, Мастера: master1@test.com, master2@test.com. Пароль: password).
   - Как запустить скрипт `race_test.sh` и что он доказывает.

3. Напиши файл `DECISIONS.md`. Укажи 5 ключевых архитектурных решений, обосновав их:
   - Использование Laravel Sail (скорость развертывания).
   - Laravel Breeze + Vue/Inertia (реактивность без написания отдельного API).
   - Пессимистичная блокировка `lockForUpdate` внутри `DB::transaction()` (надежное решение проблемы гонки).
   - Enum статусы в БД (строгая типизация состояний заявки).
   - Гибридные коммиты (Conventional Commits + русский язык для команды).

Выдай содержимое этих трех файлов.



[ 3:20 - 01.03.2026 ]

**ЗАДАЧА НА ШАГ 5 (Автотесты):**
Проект и документация готовы, но по условиям ТЗ нам нужно написать минимум 2 автотеста для проверки бизнес-логики.

**ЧТО НУЖНО СДЕЛАТЬ:**
1. Создай Feature-тест `ServiceRequestTest`.
2. Напиши внутри два теста:
   - `test_guest_can_create_service_request`: проверяет, что неавторизованный пользователь может успешно отправить POST-запрос на создание заявки, получает редирект, флеш-сообщение и запись появляется в БД со статусом 'new'.
   - `test_master_can_take_request_in_work`: проверяет, что пользователь с ролью 'master' может отправить POST-запрос на взятие заявки, и в БД статус меняется на 'assigned' с привязкой его ID.
3. Обязательно используй трейт `RefreshDatabase`, чтобы тесты не мусорили в базе.

Выдай код файла и терминальные команды для его создания и запуска (через Sail).